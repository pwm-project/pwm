            ...
            if (PWM_MAIN.JSLibrary.isEmpty(data['data'])) {
                ...
                for (var categoryIter in data['data']) {
                    ...
                    for (var settingIter in category) {
                        ...
                        bodyText += '</span>&nbsp;<span id="' + settingID + '_popup" class="btn-icon pwm-icon pwm-info-circle"></span>';
                        if (!setting['default']) {
                            bodyText += '<span class="pwm-icon pwm-pencil-square modifiedNoticeIcon" title="' + PWM_CONFIG.showString('Tooltip_ModifiedNotice') + '">&nbsp;</span>';
                        }
                        bodyText += '</div>';
                        resultCount++;
                    }
...
PWM_CFGEDIT.displaySettingsCategory = function(category) {
    ...
    if (category == 'LDAP_PROFILE') {
        htmlSettingBody += '<div style="width: 100%; text-align: center">'
            + '<button class="btn" id="button-test-LDAP_PROFILE"><span class="btn-icon pwm-icon pwm-bolt"></span>Test LDAP Profile</button>'
            + '</div>';
    } else if (category == 'DATABASE') {
        htmlSettingBody += '<div style="width: 100%; text-align: center">'
            + '<button class="btn" id="button-test-DATABASE"><span class="btn-icon pwm-icon pwm-bolt"></span>Test Database Settings</button>'
            + '</div>';
    } else if (category == 'SMS_GATEWAY') {
        htmlSettingBody += '<div style="width: 100%; text-align: center">'
            + '<button class="btn" id="button-test-SMS"><span class="btn-icon pwm-icon pwm-bolt"></span>Test SMS Settings</button>'
            + '</div>';
    } else if (category == 'HTTPS_SERVER') {
        htmlSettingBody += '<div style="width: 100%; text-align: center">'
            + '<button class="btn" id="button-test-HTTPS_SERVER"><span class="btn-icon pwm-icon pwm-bolt"></span>View Certificate Info</button>'
            + '</div>';
    }
...
PWM_CFGEDIT.drawHtmlOutlineForSetting = function(settingInfo, options) {
    ...
    var htmlBody = '<div id="outline_' + settingKey + '" class="setting_outline" style="display:none">'
        ...
        + '<div class="pwm-icon pwm-pencil-square modifiedNoticeIcon" title="' + PWM_CONFIG.showString('Tooltip_ModifiedNotice') + '" id="modifiedNoticeIcon-' + settingKey + '" style="display: none" ></div>';

    if (settingInfo['description']) {
        htmlBody += '<div class="pwm-icon pwm-question-circle icon_button" title="' + PWM_CONFIG.showString('Tooltip_HelpButton') + '" id="helpButton-' + settingKey + '"></div>';
    }

    htmlBody += '<div style="visibility: hidden" class="pwm-icon pwm-undo icon_button" title="' + PWM_CONFIG.showString('Tooltip_ResetButton') + '" id="resetButton-' + settingKey + '"></div>'
        + '</div>' // close title
