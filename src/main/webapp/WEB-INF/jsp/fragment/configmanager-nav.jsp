...
<div style="text-align: center">
  ...
  <form action="<pwm:context/><%=PwmServletDefinition.ConfigManager_Wordlists.servletUrl()%>" method="get">
    <button type="submit" class="navbutton">
      <pwm:if test="<%=PwmIfTag.TEST.showIcons%>"><span class="btn-icon pwm-icon pwm-icon-book"></span></pwm:if>
      Wordlists
    </button>
  </form>
  <form action="<pwm:context/><%=PwmServletDefinition.ConfigManager_LocalDB.servletUrl()%>" method="get">
    <button type="submit" class="navbutton">
      <pwm:if test="<%=PwmIfTag.TEST.showIcons%>"><span class="btn-icon pwm-icon pwm-icon-database"></span></pwm:if>
      LocalDB
    </button>
  </form>
</div>
