...
--%>
        ...
        <% response.setHeader("Cache-Control", "private"); response.setDateHeader("Expires", System.currentTimeMillis() + 1000 * 60 * 60 * 24); %>
        PWM_GLOBAL={};
        PWM_STRINGS={};
        ...
        function initPwmStringValues() {
            ...
            PWM_STRINGS['<%=key%>']='<%=StringEscapeUtils.escapeJavaScript(Display.getLocalizedMessage(pwmSessionHeader.getSessionStateBean().getLocale(),key,pwmApplicationHeader.getConfig()))%>';
            <% } %>
        }
        ...
        function initPwmLocaleVars() {
            ...
            createCSSClass('.flagLang_en','background-image: url(<%=request.getContextPath()%>/resources/flags/languages/en.png)');
            <% } else { %>
            createCSSClass('.flagLang_<%=loopLocale.toString()%>','background-image: url(<%=request.getContextPath()%><pwm:url url="/resources/flags/languages/"/><%=loopLocale.toString()%>.png")');
            <% } %>
