        ...
        require(["dojo","dijit/Dialog"],function(dojo){
            dojo.xhrGet({
                ...
                load: function(data){
                    ...
                    if (data['error'] == true) {
                        PWM_MAIN.showDialog(PWM_STRINGS['Title_Error'],data['errorDetail']);
                    } else {
                        PWM_MAIN.showDialog(PWM_STRINGS['Title_Success'],data['successMessage'],function(){PWM_MAIN.getObject('continueForm').submit();});
                    }
                },
...
PWM_HELPDESK.doPasswordChange = function(password, random) {
    ...
    var htmlBody = PWM_STRINGS['Field_NewPassword'] + ': <b>' + password + '</b><br/><br/><br/><div id="WaitDialogBlank"/>';
    PWM_MAIN.showWaitDialog(PWM_STRINGS['Title_PleaseWait'],htmlBody,function(){
            ...
            dojo.xhrPost({
                ...
                load: function(results){
                    ...
                    if (results['error'] == true) {
                        ...
                        bodyText += results['successMessage'];
                        bodyText += '</br></br>';
                        bodyText += PWM_STRINGS['Field_NewPassword'] + ': <b>' + password + '</b>';
                        bodyText += '<br/>';
                    }
                    bodyText += '<br/><br/><button class="btn" onclick="PWM_MAIN.getObject(\'continueForm\').submit();"> OK </button>';
                    ...
                    var theDialog = new Dialog({
                        id: 'dialogPopup',
                        title: PWM_STRINGS['Title_ChangePassword'] + ': ' + PWM_VAR['helpdesk_username'],
                        style: "width: 450px",
                        content: bodyText,
                        closable: false,
                        hide: function(){
...
PWM_HELPDESK.changePasswordPopup = function() {
    require(["dijit/Dialog"],function(){
        var bodyText = '<span id="message" class="message message-info" style="width: 400">' + PWM_STRINGS['Field_NewPassword'] + '</span>';
        if (PWM_VAR['helpdesk_setting_PwUiMode'] == 'both') {
            bodyText += '<p>&nbsp;»&nbsp; <a href="#" onclick="PWM_MAIN.clearDijitWidget(\'changepassword-popup\');PWM_HELPDESK.generatePasswordPopup();">' + PWM_STRINGS['Display_AutoGeneratedPassword'] + '</a></p>';
        }
        ...
        if (PWM_GLOBAL['setting-showStrengthMeter']) {
            ...
            bodyText += '<div id="strengthLabel">' + PWM_STRINGS['Display_StrengthMeter'] + '</div>';
        ...
        bodyText += '</tr></table>';
        bodyText += '<button name="change" class="btn" id="password_button" onclick="var pw=PWM_MAIN.getObject(\'password1\').value;PWM_MAIN.clearDijitWidget(\'changepassword-popup\');PWM_HELPDESK.doPasswordChange(pw)" disabled="true"/>' + PWM_STRINGS['Button_ChangePassword'] + '</button>';
...
PWM_HELPDESK.setRandomPasswordPopup = function() {
    var title = PWM_STRINGS['Title_ChangePassword'] + ': ' + PWM_VAR['helpdesk_username'];
    var body = PWM_STRINGS['Display_SetRandomPasswordPrompt'];
    var yesAction = function() {
        PWM_HELPDESK.doPasswordChange('[' + PWM_STRINGS['Display_Random'] +  ']',true);
    };
        ...
        if (data['error']) {
            ...
            if (PWM_MAIN.isEmpty(gridData)) {
                if (validationProps['usernameField'] && validationProps['usernameField'].length > 0) {
                    ...
                    PWM_MAIN.showSuccess(PWM_STRINGS['Display_Helpdesk']);
                }
            } else {
                if (sizeExceeded) {
                    PWM_MAIN.showError(PWM_STRINGS['Display_SearchCompleted'] + '  '  + PWM_STRINGS['Display_SearchResultsExceeded']);
                } else {
                    PWM_MAIN.showSuccess(PWM_STRINGS['Display_SearchCompleted']);
                }
            }
        }
    };
...
PWM_HELPDESK.initHelpdeskSearchPage = function() {
    PWM_MAIN.showInfo(PWM_STRINGS['Display_Helpdesk']);
