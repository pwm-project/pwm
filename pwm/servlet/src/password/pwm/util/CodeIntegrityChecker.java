...
public class CodeIntegrityChecker {
    ...
    static {
        try {
            ...
            //Message.valueOf("Dsad").getLocalizedMessage()
    ...
    public CodeIntegrityChecker(PwmApplication pwmApplication) {
            ...
            for (final Object enumValue : enumValues) {
                try {
                    ...
                    returnRecords.add(new HealthRecord(
                            HealthStatus.DEBUG,
                            HealthTopic.Integrity,
                            HealthMessage.BrokenMethod,
                            methodName.toString(),
                            errorMsg
                    ));
                }
        ...
        for (final Class loopClass : LOCALE_CHECK_CLASSES) {
            ...
            for (final Locale locale : missingKeys.keySet()) {
                for (final String key : missingKeys.get(locale)) {
                    returnSet.add(new HealthRecord(
                            HealthStatus.DEBUG,
                            HealthTopic.Integrity,
    ...
    private Map<Locale,List<String>> checkLocalesOnBundle(
        ...
        stats.totalKeys += defaultLocaleBundle.keySet().size();
        stats.totalLocales = knownLocales.size();
        for (final Locale locale : knownLocales) {
            if (!PwmConstants.DEFAULT_LOCALE.equals(locale)) {
                final String bundleFilename = checkClass.getSimpleName() + "_" + locale.toString() + ".properties";
