...
public class ActionCertViewerFunction implements SettingUIFunction
{
    ...
    private static List<Map<String, String>> makeCertDebugMap(
            ...
            final StoredConfigurationModifier modifier,
            final int iteration,
            final int webActionIter
    )
    {
        final StoredValue storedValue = modifier.newStoredConfiguration().readStoredValue( key ).orElseThrow();
        final List<ActionConfiguration> values = ( List<ActionConfiguration> ) storedValue.toNativeObject();

        final ActionConfiguration actionConfiguration = values.get( iteration );
