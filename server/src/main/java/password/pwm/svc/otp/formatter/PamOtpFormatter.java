 ...
 */
public class PamOtpFormatter implements OtpFormatter
{
    private static final PwmLogger LOGGER = PwmLogger.forClass( PamOtpFormatter.class );
    ...
    {
        final List<String> lines = splitLines( value );
        if ( lines.size() >= 2 )
        {
            ...
            if ( line.matches( "^[A-Z2-7\\=]{16}$" ) )
            {
                ...
                while ( iterator.hasNext() )
                {
                    ...
                    {
                        final OTPUserRecord.RecoveryCode code = new OTPUserRecord.RecoveryCode( line, false );
     ...
     *
     * @param otpUserRecord the record with OTP configuration
    ...
    public String stringifyRecord( final OTPUserRecord otpUserRecord )
    {
        ...
        final OTPUserRecord.Type type = otpUserRecord.getType();
        final List<OTPUserRecord.RecoveryCode> recoveryCodes = otpUserRecord.getRecoveryCodes();
        final StringBuilder pamData = new StringBuilder();
        pamData.append( secret ).append( '\n' );
        if ( OTPUserRecord.Type.HOTP == type )
        {
            pamData.append( String.format( "\" HOTP_COUNTER %d%n", otpUserRecord.getAttemptCount() ) );
        }
        else
        {
