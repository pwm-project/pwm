...
/**
 * GET a PWM endpoint and unwrap the JSON envelope.  Several peoplesearch
 * endpoints (detail, orgChartData, mailtoLinks, clientData) are GETs in the
 * legacy bundle; this mirrors {@link pwmFetch} for that verb.  The pwmFormID is
 * appended for parity with the legacy {@code getServerUrl}.
 */
export async function pwmFetchGet<T = unknown>(
    url: string,
    options: { signal?: AbortSignal } = {},
): Promise<T> {
    const global = (window as unknown as { PWM_GLOBAL?: PwmGlobal }).PWM_GLOBAL ?? {};
